...
export function PeriodTable({
  ...
  const [denom, setDenom] = useState<'billpayer' | 'person'>('billpayer');
  return (
    <div>
      <div className="mb-3 flex justify-end">
        <div className="flex rounded-md border border-navy-200 text-xs">
          {(['billpayer', 'person'] as const).map((d) => (
            <button
              key={d}
              type="button"
              onClick={() => setDenom(d)}
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {periods.map((p) => {
              const m = metrics[p];
              if (!m) return null;
              const cost = denom === 'billpayer' ? m.costPerBillpayerEur : m.costPerPersonEur;
              const save = denom === 'billpayer' ? m.savingPerBillpayerEur : m.savingPerPersonEur;
              return (
